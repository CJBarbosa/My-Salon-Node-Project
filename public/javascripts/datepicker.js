var picker = {
  ...
  attach: (opt) => {
    ...
    // (A5) POPUP DATEPICKER - ATTACH INTO HTML BODY
      ...
      inst.hWrap.onclick = (evt) => {
        ...
        //Function onChange shuld be implemented here
      };
    ...
    for (let i = 0; i < squares.length; i++) {
      ...
      if (squares[i] == "B") {
        ...
        if (squares[i][1]) {
          ...
          //If todayDate is in a not allowed to choose day, it still styled as today day.
          if (squares[i][0] == todayDate) {
            cell.classList.add("picker-d-td");
            cell.setAttribute("id", "pickersd");
          }
        }
        ...
        // ALLOWED TO CHOOSE THIS DAY
          ...
          //If not current year, days are disable
          if (year < today.getUTCFullYear()) {
            cell.classList.add("picker-d-dd");
            //If it is in a current year, months must be verified
          } else if (year == today.getUTCFullYear()) {
            //If it is not a current month, days are disable
            if (month < today.getUTCMonth() + 1) {
              cell.classList.add("picker-d-dd");
            } else if (
              //If it is a current month and days are before today, days are disable
              month == today.getUTCMonth() + 1 &&
              squares[i][0] < todayDate
            ) {
              cell.classList.add("picker-d-dd");
            } else {
              //If days are: today or after it, then days are enable
  ...
  pick: (id, day) => {
    ...
    //Clear all chosen days and leave only the current chose day
    var elems = document.querySelector(".picker-d-td");
    if (elems !== null) {
      elems.classList.remove("picker-d-td");
    }
